...
def fill_PFCs(n_PFCs, PFCs_dnn, PFCs, idx_top, pt_top, eta_top, phi_top, M_top): 
    for i,particle in enumerate(PFCs):
        if i<n_PFCs: #minore e non minore e uguale perchè parte da 0
            PFCs_dnn[idx_top, i, 0] = particle.pdgId
            PFCs_dnn[idx_top, i, 1] = particle.d0
            PFCs_dnn[idx_top, i, 2] = particle.dz
            PFCs_dnn[idx_top, i, 3] = particle.pvAssocQuality  
            PFCs_dnn[idx_top, i, 4] = particle.JetDeltaR
            PFCs_dnn[idx_top, i, 5] = particle.FatJetDeltaR
            PFCs_dnn[idx_top, i, 6] = particle.IsInJet
            PFCs_dnn[idx_top, i, 7] = particle.IsInFatJet
    return PFCs_dnn
...
if doLoop:
    if year==2018:
        ...
        data_PFC         = np.zeros((1,n_PFCs,8)) #!! setta il masssimo delle 20 da prendere e andranno usate LSTM
    ...
    for i in range(nev):
        ...
        for top_num, t in enumerate(tops):
            ...
            if year==2018:
                ...
                PFC_toappend         = np.zeros((1,n_PFCs,8))
