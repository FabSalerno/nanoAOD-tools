...
def fill_Jet_PFCs(Jet_PFCs_dnn, PFCsj0, PFCsj1, PFCsj2, idx_top): 
    for i,particle in enumerate(PFCsj0):
        if i<20:
            Jet_PFCs_dnn[idx_top, 0, i, 0] = particle.pt
            Jet_PFCs_dnn[idx_top, 0, i, 1] = particle.eta
            Jet_PFCs_dnn[idx_top, 0, i, 2] = particle.phi
            Jet_PFCs_dnn[idx_top, 0, i, 3] = particle.mass
            Jet_PFCs_dnn[idx_top, 0, i, 4] = particle.charge
            Jet_PFCs_dnn[idx_top, 0, i, 5] = particle.pdgId
            Jet_PFCs_dnn[idx_top, 0, i, 6] = particle.d0
            Jet_PFCs_dnn[idx_top, 0, i, 7] = particle.dz
            Jet_PFCs_dnn[idx_top, 0, i, 8] = particle.pvAssocQuality  
            Jet_PFCs_dnn[idx_top, 0, i, 9] = particle.JetDeltaR  
    for i,particle in enumerate(PFCsj1):
        if i<20:
            Jet_PFCs_dnn[idx_top, 1, i, 0] = particle.pt
            Jet_PFCs_dnn[idx_top, 1, i, 1] = particle.eta
            Jet_PFCs_dnn[idx_top, 1, i, 2] = particle.phi
            Jet_PFCs_dnn[idx_top, 1, i, 3] = particle.mass
            Jet_PFCs_dnn[idx_top, 1, i, 4] = particle.charge
            Jet_PFCs_dnn[idx_top, 1, i, 5] = particle.pdgId
            Jet_PFCs_dnn[idx_top, 1, i, 6] = particle.d0
            Jet_PFCs_dnn[idx_top, 1, i, 7] = particle.dz
            Jet_PFCs_dnn[idx_top, 1, i, 8] = particle.pvAssocQuality #dr  
            Jet_PFCs_dnn[idx_top, 0, i, 9] = particle.JetDeltaR 

    if hasattr(PFCsj2,"pt"):
        for i,particle in enumerate(PFCsj1):
            if i<20:
                Jet_PFCs_dnn[idx_top, 2, i, 0] = particle.pt
                Jet_PFCs_dnn[idx_top, 2, i, 1] = particle.eta
                Jet_PFCs_dnn[idx_top, 2, i, 2] = particle.phi
                Jet_PFCs_dnn[idx_top, 2, i, 3] = particle.mass
                Jet_PFCs_dnn[idx_top, 2, i, 4] = particle.charge
                Jet_PFCs_dnn[idx_top, 2, i, 5] = particle.pdgId
                Jet_PFCs_dnn[idx_top, 2, i, 6] = particle.d0
                Jet_PFCs_dnn[idx_top, 2, i, 7] = particle.dz
                Jet_PFCs_dnn[idx_top, 2, i, 8] = particle.pvAssocQuality
                Jet_PFCs_dnn[idx_top, 0, i, 9] = particle.JetDeltaR  
    return Jet_PFCs_dnn
    
def fill_FatJet_PFCs(FatJet_PFCs_dnn, PFCsfj, idx_top): 
    for i,particle in enumerate(PFCsfj):
        if i<42:
            FatJet_PFCs_dnn[idx_top, i, 0] = particle.pt
            FatJet_PFCs_dnn[idx_top, i, 1] = particle.eta
            FatJet_PFCs_dnn[idx_top, i, 2] = particle.phi
            FatJet_PFCs_dnn[idx_top, i, 3] = particle.mass
            FatJet_PFCs_dnn[idx_top, i, 4] = particle.charge
            FatJet_PFCs_dnn[idx_top, i, 5] = particle.pdgId
            FatJet_PFCs_dnn[idx_top, i, 6] = particle.d0
            FatJet_PFCs_dnn[idx_top, i, 7] = particle.dz
            FatJet_PFCs_dnn[idx_top, i, 8] = particle.pvAssocQuality  
            FatJet_PFCs_dnn[idx_top, i, 9] = particle.FatJetDeltaR   

    return FatJet_PFCs_dnn
...
if doLoop:
    if year==2018:
        ...
        data_jetPFC         = np.zeros((1,3,20,10)) #!! setta il masssimo delle 20 da prendere e andranno usate LSTM
        data_fatjetPFC      = np.zeros((1,42,10))
    ...
    for i in range(nev):
        ...
        for t in tops:
            ...
            if year==2018:
                ...
                jetPFC_toappend            = np.zeros((1,3,20,10))
                fatjetPFC_toappend         = np.zeros((1,42,10))
            ...
            if best_top_category == 0: #3j1fj
                ...
                PFCsj2           =[]
                PFCsfj           =[]
                for particle in PFCs:
                    if t.idxJet0==particle.JetIdx:
                        PFCsj0.append(particle)
                    if t.idxJet1==particle.JetIdx:
                        PFCsj1.append(particle)
                    if t.idxJet2==particle.JetIdx:
                        PFCsj2.append(particle)
                    if t.idxFatJet==particle.FatJetIdx:
                        PFCsfj.append(particle)

                jetPFC_toappend = fill_Jet_PFCs(Jet_PFCs_dnn=jetPFC_toappend, 
                                                 PFCsj0=PFCsj0, 
                                                 PFCsj1=PFCsj1, 
                                                 PFCsj2=PFCsj2, 
                                                 idx_top=0)
                
                fatjetPFC_toappend = fill_FatJet_PFCs(FatJet_PFCs_dnn=fatjetPFC_toappend, 
                                                      PFCsfj=PFCsfj, 
                                                      idx_top=0)
            ...
            data_jetPFC       = np.append(data_jetPFC,      jetPFC_toappend,            axis = 0)
            data_fatjetPFC    = np.append(data_fatjetPFC,   fatjetPFC_toappend,         axis = 0)
            ...
            if (data_jets[0, 0, 0]==0):
                ...
                data_jetPFC       = np.delete(data_jetPFC,      0, axis = 0)
                data_fatjetPFC    = np.delete(data_fatjetPFC,   0, axis = 0)
    ...
    for cat in categories:
        if "0fj" in cat :
            ...
            n = 0
        output[component][cat] = [data_jets[event_category == n], data_fatjets[event_category == n], data_jetPFC[event_category == n], data_fatjetPFC[event_category == n], data_mass[event_category == n], data_label[event_category == n]]
        #output[component][cat] = [data_jetPFC[event_category == n], data_fatjetPFC[event_category == n], data_mass[event_category == n], data_label[event_category == n]]
