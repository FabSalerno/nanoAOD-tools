...
verbose                 = True
path_to_best_hps        = "/eos/user/f/fsalerno/framework/MachineLearning/Grid_search_1/grid_search_models/PF_jets/best_hps.json"
...
for c in components:
    for cat in categories:
        ...
        dataset[c][cat][4] = np.delete(dataset[c][cat][3], ids_todrop, axis=0)
        dataset[c][cat][5] = np.delete(dataset[c][cat][3], ids_todrop, axis=0)
...
if verbose:
    for c in components:
        for cat in categories:
            ...
            print(f"Number of True Tops:        {len([i for i, x in enumerate(dataset[c][cat][5]==1) if x==True])}")
            print(f"Number of False Tops:       {len([i for i, x in enumerate(dataset[c][cat][5]==0) if x==True])}")
            ...
            print(f"Number of Tops:             {len(dataset[c][cat][4])}")
            print("\n")

    print(f"N. Top:                 {sum([len(dataset[c][cat][0]) for c in components for cat in categories])}")
    print(f"N. Top True:            {sum([len([i for i, x in enumerate(dataset[c][cat][5]==1) if x==True]) for c in components for cat in categories])}")
    print(f"N. Top False:           {sum([len([i for i, x in enumerate(dataset[c][cat][5]==0) if x==True]) for c in components for cat in categories])}")
    print("\n")

    print(f"N. Top in category 3-0: {sum([len(dataset[c]['3j0fj'][0]) for c in components])}")
    print(f"N. Top True:            {sum([len([i for i, x in enumerate(dataset[c]['3j0fj'][5]==1) if x==True]) for c in components])}")
    print(f"N. Top False:           {sum([len([i for i, x in enumerate(dataset[c]['3j0fj'][5]==0) if x==True]) for c in components])}")
    print("\n")

    print(f"N. Top in category 3-1: {sum([len(dataset[c]['3j1fj'][0]) for c in components])}")
    print(f"N. Top True:            {sum([len([i for i, x in enumerate(dataset[c]['3j1fj'][5]==1) if x==True]) for c in components])}")
    print(f"N. Top False:           {sum([len([i for i, x in enumerate(dataset[c]['3j1fj'][5]==0) if x==True]) for c in components])}")
    print("\n")

    print(f"N. Top in category 2-1: {sum([len(dataset[c]['2j1fj'][0]) for c in components])}")
    print(f"N. Top True:            {sum([len([i for i, x in enumerate(dataset[c]['2j1fj'][5]==1) if x==True]) for c in components])}")
    print(f"N. Top False:           {sum([len([i for i, x in enumerate(dataset[c]['2j1fj'][5]==0) if x==True]) for c in components])}")
    print("\n")


class trainer:
    def __init__(self, X_jet, X_fatjet, X_jet_PFC, X_fatjet_PFC, X_top, y, BestHyperParameters):
        self.X_jet               = X_jet   
        self.X_fatjet            = X_fatjet
        self.X_jet_PFC           = X_jet_PFC   
        self.X_fatjet_PFC        = X_fatjet_PFC
        self.X_top               = X_top   
        self.y                   = y
    ...
    def split(self, test_size):
        self.X_jet_train, self.X_jet_test, self.X_fatjet_train, self.X_fatjet_test, self.X_jet_PFC_train, self.X_jet_PFC_test, self.X_fatjet_PFC_train, self.X_fatjet_PFC_test, self.X_top_train, self.X_top_test, self.y_train, self.y_test = train_test_split(self.X_jet, self.X_fatjet, self.X_top, self.y, 
                                                                                                                                                                    # test_size=test_size)
                                                                                                                                                                    stratify=self.y, shuffle=True, test_size=test_size)
    
    
    def model_builder(self, InputShape_Jet, InputShape_FatJet, InputShape_FatJet_PFC, InputShape_Jet_PFC, InputShape_Top):
        # Input Layers
        ...
        fj_PFC_inputs       = tf.keras.Input(shape=(None,InputShape_FatJet_PFC,),   name="fatjetPFC")   #z
        jet_PFC_inputs      = tf.keras.Input(shape=(None,InputShape_Jet_PFC,), name="jetPFC")      #j
        ...
        ### Operations on FATJETPFC Input Layer ###
        z = Masking(mask_value=0.)(fj_PFC_inputs)
        z = BatchNormalization()(z)
        # Tune parameters for Jet Input Layer #
        z = keras.layers.LSTM(units=self.BestHyperParameters["fj_PFC_units"],
                              activation=self.BestHyperParameters["fj_PFC_activation"],
                              kernel_initializer=self.BestHyperParameters["fj_PFC_kernel_initializer"],
                              dropout=self.BestHyperParameters["fj_PFC_dropout"]
                             )(z)
        
        j = Masking(mask_value=0.)(jet_PFC_inputs)
        j = BatchNormalization()(j)
        # Tune parameters for Jet Input Layer #
        lstm_jet_PFC         =keras.layers.LSTM(units=self.BestHyperParameters["j_PFC_units"],
                              activation=self.BestHyperParameters["j_PFC_activation"],
                              kernel_initializer=self.BestHyperParameters["j_PFC_kernel_initializer"],
                              dropout=self.BestHyperParameters["j_PFC_dropout"]
                             )(j)

        j                    =keras.layers.TimeDistributed(lstm_jet_PFC)(j)
        j = keras.layers.LSTM(units=self.BestHyperParameters["j_PFC_units_2"],
                              activation=self.BestHyperParameters["j_PFC_activation_2"],
                              kernel_initializer=self.BestHyperParameters["j_PFC_kernel_initializer_2"],
                              dropout=self.BestHyperParameters["j_PFC_dropout_2"]
                             )(j)
        ...
        self.model   = tf.keras.Model(inputs=[fj_inputs, jet_inputs, fj_PFC_inputs, jet_PFC_inputs, top_inputs], outputs=outputs)
    ...
    def training(self, validation_split, epochs=50, batch_size=1, verbose=True, save_model=False, path_to_model=None):
        self.callbacks()
        self.model_builder(self.X_jet_train.shape[2], self.X_fatjet_train.shape[1], self.X_jet_PFC_train.shape[3], self.X_fatjet_PFC_train.shape[2], self.X_top_train.shape[1])
        # Dataset Balancing #
        ...
        self.history  = self.model.fit({"fatjet": self.X_fatjet_train, "jet": self.X_jet_train, "fatjetPFC": self.X_fatjet_PFC_train, "jetPFC": self.X_jet_PFC_train, "top": self.X_top_train}, self.y_train,
    ...
    def evaluate(self, X_jet_test=None, X_fatjet_test=None,  X_jet_PFC_test=None, X_fatjet_PFC_test=None, X_top_test=None, y_test=None):
        if (X_jet_test is None) and (X_fatjet_test is None) and (X_top_test is None) and (y_test is None):
            self.eval_result = self.model.evaluate({"fatjet": self.X_fatjet_test, "jet": self.X_jet_test, "fatjetPFC": self.X_fatjet_PFC_test, "jetPFC": self.X_jet_PFC_test, "top": self.X_top_test}, self.y_test)
            return self.eval_result
        else:
            eval_result      = self.model.evaluate({"fatjet": X_fatjet_test, "jet": X_jet_test,  "fatjetPFC": X_fatjet_PFC_test, "jetPFC": X_jet_PFC_test,  "top": X_top_test}, y_test)
            return eval_result
    ...
    def predict(self, X_jet_train=None, X_fatjet_train=None, X_top_train=None, X_jet_test=None, X_fatjet_test=None, X_top_test=None):
        if (X_jet_train is None) and (X_fatjet_train is None) and (X_top_train is None) and (X_jet_test is None) and (X_fatjet_test is None) and (X_top_test is None):
            self.y_pred_train = self.model.predict({"fatjet": self.X_fatjet_train, "jet": self.X_jet_train, "fatjetPFC": self.X_fatjet_PFC_train, "jetPFC": self.X_jet_PFC_train, "top": self.X_top_train})
            self.y_pred_test  = self.model.predict({"fatjet": self.X_fatjet_test, "jet": self.X_jet_test, "fatjetPFC": self.X_fatjet_PFC_train, "jetPFC": self.X_jet_PFC_train, "top": self.X_top_test})
        else:
            y_pred_train      = self.model.predict({"fatjet": X_fatjet_train, "jet": X_jet_train, "fatjetPFC": X_fatjet_PFC_train, "jetPFC": X_jet_PFC_train, "top": X_top_train})
            y_pred_test       = self.model.predict({"fatjet": X_fatjet_test, "jet": X_jet_test, "fatjetPFC": X_fatjet_PFC_train, "jetPFC": X_jet_PFC_train, "top": X_top_test})
...
X_jet_PFC                 = np.concatenate([dataset[c][cat][2] for c in samples for cat in categories]) # here we use only the samples selected by the user
X_fatjet_PFC              = np.concatenate([dataset[c][cat][3] for c in samples for cat in categories]) # here we use only the samples selected by the user
...
data                      = X_jet, X_fatjet, X_jet_PFC, X_fatjet_PFC, X_top, y
...
if verbose:
    ...
    print(f"\tX_jet_PFC shape:        {X_jet_PFC.shape}")
    print(f"\tX_fatjet_PFC shape:     {X_fatjet_PFC.shape}")
