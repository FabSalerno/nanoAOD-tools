...
example     = 'python3 training_Run3_hotvr.py -s QCD_HT1000_2018,QCD_HT1500_2018,QCD_HT2000_2018,QCD_HTInf_2018,TT_Mtt1000toInf_2018,TT_Mtt700to1000_2018,TT_semilep_2018 -i /eos/user/f/fsalerno/framework/MachineLearning/Training_HOTVR_2018_1/trainingSet.pkl -m /eos/user/f/fsalerno/framework/MachineLearning/Training_HOTVR_2018_1/model.h5 -j /eos/user/f/fsalerno/framework/MachineLearning/Training_HOTVR_2018_1/score_thresholds.json -g /eos/user/f/fsalerno/framework/MachineLearning/Training_HOTVR_2018_1/graphics'
...
verbose                 = True
path_to_best_hps        = "/eos/user/f/fsalerno/framework/MachineLearning/Grid_search_1/grid_search_models/HOTVR_new_def/best_hps.json"

print(path_to_graphics_folder)
...
if verbose:
    for c in components:
        for cat in categories:
            ...
            print(f"Number of HOTVRJets:    {len(dataset[c][cat][2])}")
    ...
    print(f"N. Top in category 3-0-0: {sum([len(dataset[c]['3j0fj0hvrj'][0]) for c in components])}")
    print(f"N. Top True:              {sum([len([i for i, x in enumerate(dataset[c]['3j0fj0hvrj'][4]==1) if x==True]) for c in components])}")
    print(f"N. Top False:             {sum([len([i for i, x in enumerate(dataset[c]['3j0fj0hvrj'][4]==0) if x==True]) for c in components])}")
    print("\n")

    print(f"N. Top in category 3-1-0: {sum([len(dataset[c]['3j1fj0hvrj'][0]) for c in components])}")
    print(f"N. Top True:              {sum([len([i for i, x in enumerate(dataset[c]['3j1fj0hvrj'][4]==1) if x==True]) for c in components])}")
    print(f"N. Top False:             {sum([len([i for i, x in enumerate(dataset[c]['3j1fj0hvrj'][4]==0) if x==True]) for c in components])}")
    print("\n")

    print(f"N. Top in category 3-0-1: {sum([len(dataset[c]['3j0fj1hvrj'][0]) for c in components])}")
    print(f"N. Top True:              {sum([len([i for i, x in enumerate(dataset[c]['3j0fj1hvrj'][4]==1) if x==True]) for c in components])}")
    print(f"N. Top False:             {sum([len([i for i, x in enumerate(dataset[c]['3j0fj1hvrj'][4]==0) if x==True]) for c in components])}")
    print("\n")

    print(f"N. Top in category 2-1-0: {sum([len(dataset[c]['2j1fj0hvrj'][0]) for c in components])}")
    print(f"N. Top True:              {sum([len([i for i, x in enumerate(dataset[c]['2j1fj0hvrj'][4]==1) if x==True]) for c in components])}")
    print(f"N. Top False:             {sum([len([i for i, x in enumerate(dataset[c]['2j1fj0hvrj'][4]==0) if x==True]) for c in components])}")
    print("\n")

    print(f"N. Top in category 2-0-1: {sum([len(dataset[c]['2j0fj1hvrj'][0]) for c in components])}")
    print(f"N. Top True:              {sum([len([i for i, x in enumerate(dataset[c]['2j0fj1hvrj'][4]==1) if x==True]) for c in components])}")
    print(f"N. Top False:             {sum([len([i for i, x in enumerate(dataset[c]['2j0fj1hvrj'][4]==0) if x==True]) for c in components])}")
    print("\n")


class trainer:
    def __init__(self, X_jet, X_fatjet, X_hotvrjet, X_top, y, BestHyperParameters):
        self.X_jet               = X_jet   
        self.X_fatjet            = X_fatjet
        self.X_hotvrjet          = X_hotvrjet
        self.X_top               = X_top   
        self.y                   = y
    ...
    def split(self, test_size):
        self.X_jet_train, self.X_jet_test, self.X_fatjet_train, self.X_fatjet_test, self.X_hotvrjet_train, self.X_hotvrjet_test, self.X_top_train, self.X_top_test, self.y_train, self.y_test = train_test_split(self.X_jet, self.X_fatjet, self.X_hotvrjet, self.X_top, self.y, 
                                                                                                                                                                    # test_size=test_size)
                                                                                                                                                                    stratify=self.y, shuffle=True, test_size=test_size)
    
    
    def model_builder(self, InputShape_Jet, InputShape_FatJet, InputShape_HotvrJet, InputShape_Top):
        # Input Layers
        ...
        ### Operations on HOTVRJET Input Layer ### 
        # x           = Masking(mask_value=0.)(fj_inputs)
        # x           = BatchNormalization()(x)
        x           = BatchNormalization()(hvrj_inputs)
        # Tune parameters for HotvrJet Input Layer # 
        #USO FATJET PER ORA
        x           = Dense(units=self.BestHyperParameters["hvrj_units"],
                            activation=self.BestHyperParameters["hvrj_activation"],
                            kernel_initializer=self.BestHyperParameters["hvrj_kernel_initializer"]
                            )(x)
        ...
        y           = Dense(units=self.BestHyperParameters["fj_units"],
        ...
        z = keras.layers.LSTM(units=self.BestHyperParameters["j_units"],
        ...
        self.model   = tf.keras.Model(inputs=[hvrj_inputs, fj_inputs, jet_inputs, top_inputs], outputs=outputs)
    ...
    def training(self, validation_split, epochs=50, batch_size=1, verbose=True, save_model=False, path_to_model=None):
        self.callbacks()
        self.model_builder(self.X_jet_train.shape[2], self.X_fatjet_train.shape[1], self.X_hotvrjet_train.shape[1], self.X_top_train.shape[1])
        # Dataset Balancing #
        ...
        self.history  = self.model.fit({"hotvrjet": self.X_hotvrjet_train, "fatjet": self.X_fatjet_train, "jet": self.X_jet_train, "top": self.X_top_train}, self.y_train,
    ...
    def evaluate(self, X_jet_test=None, X_fatjet_test=None, X_hotvrjet_test=None, X_top_test=None, y_test=None):
        if (X_jet_test is None) and (X_fatjet_test is None) and (X_hotvrjet_test is None) and (X_top_test is None) and (y_test is None):
            self.eval_result = self.model.evaluate({"hotvrjet": self.X_hotvrjet_test, "fatjet": self.X_fatjet_test, "jet": self.X_jet_test, "top": self.X_top_test}, self.y_test)
            return self.eval_result
        else:
            eval_result      = self.model.evaluate({"hotvrjet": self.X_hotvrjet_test, "fatjet": X_fatjet_test, "jet": X_jet_test, "top": X_top_test}, y_test)
            return eval_result
    

    def predict(self, X_jet_train=None, X_fatjet_train=None, X_hotvrjet_train=None, X_top_train=None, X_jet_test=None, X_fatjet_test=None, X_hotvrjet_test=None, X_top_test=None):
        if (X_jet_train is None) and (X_fatjet_train is None) and (X_hotvrjet_train is None) and (X_top_train is None) and (X_jet_test is None) and (X_fatjet_test is None) and (X_hotvrjet_test is None) and (X_top_test is None):
            self.y_pred_train = self.model.predict({"hotvrjet": self.X_hotvrjet_train, "fatjet": self.X_fatjet_train, "jet": self.X_jet_train, "top": self.X_top_train})
            self.y_pred_test  = self.model.predict({"hotvrjet": self.X_hotvrjet_test, "fatjet": self.X_fatjet_test, "jet": self.X_jet_test, "top": self.X_top_test})
        else:
            y_pred_train      = self.model.predict({"hotvrjet": self.X_hotvrjet_train, "fatjet": X_fatjet_train, "jet": X_jet_train, "top": X_top_train})
            y_pred_test       = self.model.predict({"hotvrjet": self.X_hotvrjet_test, "fatjet": X_fatjet_test, "jet": X_jet_test, "top": X_top_test})
...
X_hotvrjet                = np.concatenate([dataset[c][cat][2] for c in samples for cat in categories]) # here we use only the samples selected by the user
...
data                      = X_jet, X_fatjet, X_hotvrjet, X_top, y
...
if verbose:
    ...
    print(f"\tX_HOTVRjet shape:       {X_hotvrjet.shape}")
