...
def sub_writer(component,n_PFCs,pt_cut):
    ...
    #f.write("requirements = Cpus >= 4 && (PartitionableSlot || Cpus <= 8)")
...
def sh_writer(year, component, n_PFCs, pt_cut, inFile_to_open, nev, path_to_pkl, select_top_over_threshold, thr):
...
######## LAUNCH CONDOR ########
n_PFCs=20  #number of PFCs 
pt_cut=300 #only top with pt>pt_cut are selected
year                        = 2022
...
if year==2018:
    ...
    path_to_training_folder = f"/eos/user/f/fsalerno/framework/MachineLearning/Training_PF_2022_1_jets_{n_PFCs}_boosted_{pt_cut}_pt"
...
for dat in datasets:
    ...
    sh_writer(year=year,
              component=dat,
              inFile_to_open="/eos/user/f/fsalerno/Data/PF/topcand_big/nano_mcRun3_ttsl1_topcand_TROTA_ttsl1.root",
              nev=nev,
