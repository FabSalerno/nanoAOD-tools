...
X_jet_PFC       = np.concatenate([dataset[c][cat][2] for c in components for cat in categories]) 
X_fatjet_PFC    = np.concatenate([dataset[c][cat][3] for c in components for cat in categories]) 
X_top           = np.concatenate([dataset[c][cat][4] for c in components for cat in categories])         
y               = np.concatenate([dataset[c][cat][5] for c in components for cat in categories])




X_jet_train, X_jet_test, X_fatjet_train, X_fatjet_test, X_jet_PFC_train, X_jet_PFC_test, X_fatjet_PFC_train, X_fatjet_PFC_test, X_top_train, X_top_test, y_train, y_test = train_test_split(X_jet, X_fatjet, X_jet_PFC, X_fatjet_PFC, X_top, y, stratify=y, shuffle=True, test_size=0.3)
print(X_jet_train.shape, X_jet_test.shape, X_fatjet_train.shape, X_fatjet_test.shape,  X_jet_PFC_train.shape, X_jet_PFC_test.shape, X_fatjet_PFC_train.shape, X_fatjet_PFC_test.shape, X_top_train.shape, X_top_test.shape, y_train.shape, y_test.shape)
...
InputShape_FatJet_PFC=X_fatjet_PFC_train.shape[2]
InputShape_Jet_PFC=X_jet_PFC_train.shape[3]
print("PROVA InputShape_Jet_PFC",InputShape_Jet_PFC)
...
def model_builder(hp):
    ...
    jet_PFC_inputs      = tf.keras.Input(shape=(None,None,InputShape_Jet_PFC,), name="jetPFC")           #j ???
    top_inputs          = tf.keras.Input(shape=(InputShape_Top,),      name="top")                  #t
    print("\nJet",jet_inputs ,"FatJet",fj_inputs ,"Jet_PFC",jet_PFC_inputs,"FatJet_PFC",fj_PFC_inputs,"top",top_inputs)
...
#     y = keras.layers.SimpleRNN(5, activation="relu", kernel_initializer="random_normal")(y)
    ...
    # Tune parameters for Jet Input Layer #
    # j_units              = hp.Int("j_units", min_value=1, max_value=20, step=1)
    fj_PFC_units              = hp.Int("fj_PFC_units", min_value=1, max_value=10, step=1)
    fj_PFC_activation         = hp.Choice("fj_PFC_activation", values=["relu", "sigmoid", "tanh"])
    # j_activation         = hp.Choice("j_activation", values=["relu"])
#     j_kernel_initializer = hp.Choice("j_kernel_initializer", values=["glorot_uniform", "glorot_normal", "random_uniform", "random_normal", "zeros", "ones"])
    fj_PFC_kernel_initializer = hp.Choice("fj_PFC_kernel_initializer", values=["random_uniform", "random_normal"])
    fj_PFC_dropout            = hp.Choice("fj_PFC_dropout", values=list(np.arange(0,1,0.3)))
#     y = tf.keras.layers.SimpleRNN(5, activation="relu", kernel_initializer="random_normal")(y)
    z                    = tf.keras.layers.LSTM(units=fj_PFC_units,
                                             activation=fj_PFC_activation,
                                             kernel_initializer=fj_PFC_kernel_initializer,
                                             dropout=fj_PFC_dropout
                                            )(z)
    print("Output shape after FATJET_PFC LSTM:", z.shape)
    ### Operations on JETPFC Input Layer ###
    j = Masking(mask_value=0.)(jet_PFC_inputs)
    j = BatchNormalization()(j)
    # Tune parameters for Jet Input Layer #
    # j_units              = hp.Int("j_units", min_value=1, max_value=20, step=1)
    j_PFC_units              = hp.Int("j_PFC_units", min_value=1, max_value=10, step=1)
    j_PFC_activation         = hp.Choice("j_PFC_activation", values=["relu", "sigmoid", "tanh"])
    # j_PFC_activation         = hp.Choice("j_PFC_activation", values=["relu"])
#     j_PFC_kernel_initializer = hp.Choice("j_PFC_kernel_initializer", values=["glorot_uniform", "glorot_normal", "random_uniform", "random_normal", "zeros", "ones"])
    j_PFC_kernel_initializer = hp.Choice("j_PFC_kernel_initializer", values=["random_uniform", "random_normal"])
    j_PFC_dropout            = hp.Choice("j_PFC_dropout", values=list(np.arange(0,1,0.3)))
#     y = tf.keras.layers.SimpleRNN(5, activation="relu", kernel_initializer="random_normal")(y)
    print("Input shape before TimeDistributed LSTM:", j.shape)
    lstm_jet_PFC         =tf.keras.layers.LSTM(units=j_PFC_units,
                                             activation=j_PFC_activation,
                                             kernel_initializer=j_PFC_kernel_initializer,
                                             dropout=j_PFC_dropout)
    print("Input shape before TimeDistributed LSTM output:", j.shape)
    #j                    =tf.keras.layers.TimeDistributed(lstm_jet_PFC, input_shape=(3, 20, 9))(j) 
    j                    =tf.keras.layers.TimeDistributed(lstm_jet_PFC)(j)
    #j                    = tf.keras.layers.LSTM(units=j_units,
    #                                        activation=j_activation,
    #                                        kernel_initializer=j_kernel_initializer,
    #                                         dropout=j_dropout
    #                                       )(j)
    print("Output shape after TimeDistributed LSTM:", j.shape)

    j_PFC_units_2             = hp.Int("j_PFC_units_2", min_value=1, max_value=10, step=1)
    j_PFC_activation_2        = hp.Choice("j_PFC_activation_2", values=["relu", "sigmoid", "tanh"])
    j_PFC_kernel_initializer_2 = hp.Choice("j_PFC_kernel_initializer_2", values=["random_uniform", "random_normal"])
    j_PFC_dropout_2           = hp.Choice("j_PFC_dropout_2", values=list(np.arange(0,1,0.3)))

    j                    =tf.keras.layers.LSTM(units=j_PFC_units_2,
                                             activation=j_PFC_activation_2,
                                             kernel_initializer=j_PFC_kernel_initializer_2,
                                             dropout=j_PFC_dropout_2)(j)
    
    print("Output shape after last LSTM:", j.shape)
...
#     x = Dropout(dropout)(x)
    ...
    model   = tf.keras.Model(inputs=[fj_inputs, jet_inputs, fj_PFC_inputs, jet_PFC_inputs, top_inputs], outputs=outputs)
...
tuner.search({"fatjet": X_fatjet_train, "jet": X_jet_train, "fatjetPFC": X_fatjet_PFC_train, "jetPFC": X_jet_PFC_train, "top": X_top_train}, y_train,
