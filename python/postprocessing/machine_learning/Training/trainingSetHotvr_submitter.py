...
def sh_writer(year, component, inFile_to_open, nev, path_to_pkl, select_top_over_threshold, thr):
    ...
    if select_top_over_threshold:
        f.write(f"python3 trainingSetHotvr.py -year {year} -component {component} -inFile_to_open {inFile_to_open} -nev {nev} -path_to_pkl {path_to_pkl} -select_top_over_threshold -thr {thr}\n")
    else:
        f.write(f"python3 trainingSetHotvr.py -year {year} -component {component} -inFile_to_open {inFile_to_open} -nev {nev} -path_to_pkl {path_to_pkl}\n")
...
for dat in datasets:
    ...
    sh_writer(year=year,
              component=dat,
              inFile_to_open="/eos/user/f/fsalerno/Data/HOTVR/training_j_in_HVRj/Merged_Friend_HOTVR_qcd_ht_1500_MC2018.root",
              nev=nev,
