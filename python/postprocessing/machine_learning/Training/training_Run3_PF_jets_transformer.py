...
from tensorflow.keras.layers import Dense, Dropout, LSTM, concatenate, GRU,Masking, Activation, TimeDistributed, Conv1D, BatchNormalization, MaxPooling1D, Reshape, Flatten, Input
...
from tensorflow.keras.layers import (
    MultiHeadAttention, Dense, LayerNormalization, Dropout, Input, Add, Masking, BatchNormalization
)
from tensorflow.keras.models import Model

def transformer_encoder(inputs, num_heads=4, key_dim=13, ff_dim=64, dropout=0.3):
    ...
    ffn_output = Dense(key_dim)(ffn_output)  # Project back to key_dim
    output = Add()([attention_output, ffn_output])  # Residual connection
...
parser.add_argument('-n', '--nPFCs',      dest = 'nPFCs',     required = True,   default = 20,                              type = int,      help = 'number of particles used for training (default "20")')
...
class trainer:
    ...
    def model_builder(self, InputShape_Jet, InputShape_FatJet, InputShape_PFC, InputShape_Top):
        ...
        #Conv1D layer
        print("1",z.shape)
        z = transformer_encoder(z, num_heads=4, key_dim=13, ff_dim=64, dropout=0.3)
        z = Flatten()(z)
        print("3",z.shape)
