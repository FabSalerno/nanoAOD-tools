...
"""def select_jets(selection, j, fj):
    if selection == 1:
        j_sel = lambda j: j.pt> 40
        fj_sel = lambda j: j.pt> 200
        gj = list(filter(j_sel,j))
        gfj = list(filter(j_sel,fj))
        if ((len(gj)>2) or (len(gj)>1 and len(gfj)>0)):
            block = True
        else:
            block = False
    else:
...
class nanoTopcand(Module):
    def __init__(self, isMC=1, selection=0):
        self.isMC = isMC
    ...
    def analyze(self, event):
        ...
        if self.selection == True:
            goodjets_0, goodfatjets_0 = presel(jets, fatjets)
            goodjets = list(filter(lambda j: j.pt> 30, goodjets_0))
            goodfatjets = list(filter(lambda j: j.pt> 40, goodfatjets_0))

            if not ((len(goodjets)>2) or (len(goodjets)>1 and len(goodfatjets)>0)): return False
        else:
            goodjets, goodfatjets = presel(jets, fatjets)

        """goodjets_0, goodfatjets_0 = presel(jets, fatjets)
        block, goodjets, goodfatjets = select_jets(selection=self.selection, j=goodjets_0, fj=goodfatjets_0)
